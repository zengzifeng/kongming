...
flowchart TD
    ...
    CC -->|否| Acq["_acquire_machines 腾挪机器<br/>总量守恒 / KSCC最小保留 / 供出上限 min(donatable, native_free//rate)<br/>产能非对称：目标按目标速率、源释放按源速率"]
    CC -->|是| Take{"take = min(need, free) > 0？"}
    Acq --> Take
    Take -->|否| Rej2["拒收 self_cluster_capacity_insufficient"]
    Take -->|是| Accept["扣 native/received（先 received 后 native）；客户进 accepted<br/>就地更新 machine_count → machines_after"]
...
- **原生/接收冗余分账**（与 realtime 一致）：`native_free[集群] = current_redundant_tpm`（原生空闲，可被 commit 消耗、也可背书供出机器）；`received_free[集群] = 0` 起（腾挪接收到的容量，只可被 commit 消耗、**永不背书供出**）；`donatable` = 可供出机器。
- 逐候选（高密度优先）：
  - `servable` = 可服务集群；无 → 拒 `no_servable_cluster`。
  - `free = Σ(native_free + received_free)`（该客户可服务集群的当前空闲）；`need = peak_vendor_gap`。
  - 若 `need − free > EPS`：`_acquire_machines` 从源集群腾挪机器到目标（可服务集群里单机产能最高者），产能非对称计入（目标 `machines×目标速率` 记入目标 `received_free`、源释放 `machines×源速率` 从源 `native_free` 扣），就地更新 `machine_count`，腾挪后**重算 free**；供出上限 `min(donatable, ⌊native_free/源速率⌋)`（只背书原生空闲，杜绝“同一份容量既算 servable free 又算腾挪新增”的双重计数），受 KSCC 最小保留约束。**无需 donor/receiver 互斥**：接收容量只进 `received_free`、不背书供出，真 relay 结构上不可能，故允许集群既供又收（A→B & C→A）。
  - `take = min(need, free)`；`take ≤ EPS` → 拒 `self_cluster_capacity_insufficient`。否则从 `native_free/received_free` 扣减 `take`（**先扣 received 后扣 native**，把原生容量尽量留作可供出），客户进 `accepted`。
